...
class LegoClassifier:
    ...
    def __init__(self, data_dir, out_dir, epochs, batch_size):
    ...
    # Data loading
    def _load_split(self, subset_dir, pad: bool = False):
        """
        Loads train/val splits from a subdirectory, then splits the val
        ...
        # this means that input stays the same shape, so nothing breaks the vgg model
    ...
    def load_data(self):
        ...
        self.class_names = sorted([item.name for item in (self.data_dir / "cropped").glob("*") if item.is_dir()]) # forcing ordered classes (they are the same in both directories)
    ...
    def save_report(self, y_true, y_pred, name):
        """
        writes a classification report to the out directory.
        """
        report = classification_report(y_true, y_pred, target_names=self.class_names)
        path = self.out_dir / f"report_{name}.txt"
        path.write_text(f"#### Classification report – {name} ####\n\n{report}")

    def save_history_plot(self, history, name):
        """
    ...
    def run(self, subset="cropped", model="lenet"):
        """
        Convenience wrapper that calls the full pipeline.
        subset: "base" | "cropped" | "both"
        model: "lenet" | "vgg" | "both"
        """
        self.load_data()

        #creating a model dictionary depending on arguments from argparse
        models = {}
        ...
        # same for choice of training data
        datasets = []
        ...
        for model_name, model_obj, data_name, train, val, test in targets:
            ...
            self.evaluate(test) # eval of model object on held out test data
